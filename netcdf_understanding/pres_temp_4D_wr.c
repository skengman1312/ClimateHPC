...
int main()
{
   ...
   int dimids[NDIMS];// equal 4

   /* The start and count arrays will tell the netCDF library where to
      write our data. */
   size_t start[NDIMS], count[NDIMS]; // size_t is the unsigned integer type of the result
   // equal 4
   /* Program variables to hold the data we will write out. We will only
      need enough space to hold one timestep of data; one record. */
   float pres_out[NLVL][NLAT][NLON];// NLvl =1,NLAT=6,NLON=12
   float temp_out[NLVL][NLAT][NLON];

   /* These program variables hold the latitudes and longitudes. */
   float lats[NLAT], lons[NLON];// two arrays containine everything
   ...
   for (lat = 0; lat < NLAT; lat++)
      lats[lat] = START_LAT + 5.*lat;// start is 25
   for (lon = 0; lon < NLON; lon++)
      lons[lon] = START_LON + 5.*lon;// start -125
   
   for (lvl = 0; lvl < NLVL; lvl++)
      for (lat = 0; lat < NLAT; lat++)
	 for (lon = 0; lon < NLON; lon++)
	 {
	    pres_out[lvl][lat][lon] = SAMPLE_PRESSURE + i;// smaple preasure = 900
	    temp_out[lvl][lat][lon] = SAMPLE_TEMP + i++;// sample temperature is equal to9 
	 }

   /* Create the file. */
   if ((retval = nc_create(FILE_NAME, NC_CLOBBER, &ncid)))// ncclober to overwrite the file
      ERR(retval);
   ...
   if ((retval = nc_def_dim(ncid, REC_NAME, NC_UNLIMITED, &rec_dimid)))
      ERR(retval);// you see it to unlimitied 

   /* Define the coordinate variables. We will only define coordinate
      variables for lat and lon. 
      Ordinarily we would need to provide
      an array of dimension IDs for each variable's dimensions, but
      since coordinate variables only have one dimension, 
      we can simply provide the address of that dimension ID (&lat_dimid) and
   ...
   /*
      ...
      const char * 	name,
      nc_type 	xtype,
      int 	ndims, it is set to 1 1 specifies a vector, 	Number of dimensions for the variable. 
      const int * 	dimidsp, 	Vector of ndims dimension IDs corresponding to the variable dimension. id of 
      the dimenison that you created previously. So you need a dimension for each variable
      int * 	varidp )	 	Pointer to location for the returned variable ID.
   */
