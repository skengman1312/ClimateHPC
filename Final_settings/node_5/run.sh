...
#PBS -l select=5:ncpus=5:mem=8gb 
#PBS -j oe
...
mpirun -np 25 -prepend-rank  $(pwd)/reading_u_hybrid.out
...
mpiexec -n 5 --report-bindings --map-by node:pe=5 --bind-to core  $(pwd)/reading_u_hybrid_v1.out
echo "Ending the comm splitting version"
...
mpiexec -n 5 --report-bindings --map-by node:pe=5 --bind-to core  $(pwd)/reading_u_hybrid_v2.out
